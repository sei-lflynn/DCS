...
//Enum used to store every valid ECS. Currently used only in validation, but could (and maybe should) be used for general ecs storage 
public enum ECSTYPE{
    ...
    public static  ECSTYPE fromBytes(byte[] value){
        Optional<ECSTYPE> result = Arrays.stream(ECSTYPE.class.getEnumConstants()).filter(ecs ->  Arrays.equals(ecs.value,value)).findFirst();
        if (result.isPresent() ){
            return result.get();
        }else{
            return null;
