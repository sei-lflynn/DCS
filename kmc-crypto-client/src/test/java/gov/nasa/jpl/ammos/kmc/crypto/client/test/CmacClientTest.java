...
/**
 * Unit tests for Integrity Check with CMAC algorithms using the Crypto Service.
 *
 *
 */
public class CmacClientTest {
    ...
    public final void testBadMetadata()
        ...
        is.reset();
        String invalidKeyAlg = "metadataType:IntegrityCheckMetadata,"
                + "keyRef:kmc/test/AES256,cryptoAlgorithm:AESCMACx,keyLength:256,"
                + "integrityCheckValue:" + icv;
        try {
            verifier.verifyIntegrityCheckValue(is, invalidKeyAlg);
            fail("Expected KmcCryptoException not received.");
        } catch (KmcCryptoException e) {
    ...
    @Test
    public final void testWrongKeyAlgorithm()
            throws KmcCryptoManagerException, KmcCryptoException, UnsupportedEncodingException {
        String testString = "This is the test string for ICV.";
        InputStream bis = new ByteArrayInputStream(testString.getBytes("UTF-8"));
        try {
            IcvCreator icvCreator = cryptoManager.createIcvCreator(WRONG_ALGORITHM_KEYREF);
            icvCreator.createIntegrityCheckValue(bis);
            fail("Expected KmcCryptoException not received.");
        } catch (KmcCryptoException e) {
            assertEquals(KmcCryptoErrorCode.CRYPTO_ALGORITHM_ERROR, e.getErrorCode());
        }
    ...
    */
    ...
    @Test
    public final void testNonExistKey() throws KmcCryptoManagerException, UnsupportedEncodingException  {
        try {
