...
public class AEOffsetAndIVTest {
    ...
    public static void setUp() throws Exception {
        ...
        cryptoManager = CryptoTestUtils.getTestingCryptoManager();
    ...
    public final void testAESwithIVofWrongSize()
        ...
        try {
            ...
            assertTrue(e.getMessage().contains("Input IV has size " + String.valueOf(AES_BLOCK_SIZE) +
                    " bytes, expected GCM IV size = " + String.valueOf(GCM_IV_LENGTH)));
        }
    }
    ...
    public final void testAESwithIVofNotUrlSafeBase64()
        ...
        try {
            ...
            assertEquals(KmcCryptoErrorCode.INVALID_INPUT_VALUE, e.getErrorCode());
            assertTrue(e.getMessage().contains("Input IV has size 0 bytes, expected GCM IV size = 12"));
        }
    }
    ...
    public final void testAESwithIVofNull()
        ...
        try {
            ...
            assertEquals(KmcCryptoErrorCode.INVALID_INPUT_VALUE, e.getErrorCode());
            assertTrue(e.getMessage().contains("Input IV has size 3 bytes, expected GCM IV size = 12"));
        }
    }
    ...
    public final void testGCMreuseIV()
        ...
        try {
            ...
            // reusing IV does not fail because the Crypto Service creates a new Encrypter for each encrypt request.
        } catch (KmcCryptoException e) {
            Throwable cause = e.getCause();
            assertTrue(cause instanceof java.security.InvalidAlgorithmParameterException);
            assertTrue(e.getMessage().contains("Cannot reuse iv for GCM encryption"));
            assertEquals(KmcCryptoErrorCode.INVALID_INPUT_VALUE, e.getErrorCode());
        }
    }
    ...
    public final void testCorruptedTag()
        ...
        // decrypt with corrupted AEAD tag (tag is in the last 16 bytes of ciphertext)
